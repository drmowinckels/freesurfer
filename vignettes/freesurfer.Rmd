...
1. It looks for the R option `getOption("freesurfer.path")`.   
...
If your Freesurfer installation isn't in a standard location, you can manually specify its path using `options(freesurfer.path="/path/to/freesurfer")`.
...
```r
options(
    freesurfer.path = "/path/to/freesurfer",
...
FreeSurfer, like many command-line tools, relies on specific environment variables and setup scripts to function correctly.
...
`get_fs()` automates this complex setup process, allowing you to run FreeSurfer commands seamlessly from R without manually configuring your shell for each session.
...
1.  **Setting Output Format (`FSF_OUTPUT_FORMAT`)**:
