...
As this is a commonly-used process, we have wrapped these two steps into the `read_mgz` and `readmgh` functions, which combine the `mri_convert` and `readnii` functions.  Here we show that these steps are equivalent to the `read_mgz` function:
