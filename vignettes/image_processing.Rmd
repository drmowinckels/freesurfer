...
# Helper function for fslr dependency check
have_fsl <- function() {
  if ("fslr" %in% installed.packages()) {
    return(fslr::have_fsl())
  }
  FALSE
}
...
The `readmgz()` function converts this `.mgz` file to NIfTI format and then read it into R:
...
```{r mri_plot2, eval = have_both(), fig.cap = "Slices of the Bert T1 image after reorientation", fig.asp=0.8}
...
In addition to `readmgz` and `readmgh`, we also provide a `readmnc` wrapper function for directly reading MINC files after they've been converted to NIfTI.
